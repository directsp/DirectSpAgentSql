...
BEGIN
    ...
    -- Declaring pattern
    DECLARE @pattern_offset TSTRING = LOWER(tCodeQuality.Test_@removeWhitespaces('OFFSET @recordIndex ROWS FETCH NEXT @recordCount ROWS ONLY'));
    DECLARE @pattern_pageIndex TSTRING = LOWER(tCodeQuality.Test_@removeWhitespaces('@recordIndex = @recordIndex'));
    DECLARE @msg TSTRING;
