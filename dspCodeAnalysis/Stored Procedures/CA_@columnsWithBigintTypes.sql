...
CREATE PROC dspCodeAnalysis.CA_@columnsWithBigintTypes
AS
BEGIN
    WITH BigIntColumns
    ...
    SELECT  DISTINCT S.name AS schemaName, P.name AS procedureName, SM.definition, BC.columnName
     ...
     WHERE  dspCodeAnalysis.CA_@removeWhitespacesBig(REPLACE(SM.definition, 'INTO', '')) LIKE '%' + BC.columnName + 'INT%' --
        AND S.name <> 'tSQLt';
END;